...
#define CALIBRATE_ZERO_BUF_SIZE 50
...
double gps_time = 0.0, gps_zero_alt = 0.0;
double gps_flying_alt = 0.0, gps_flying_alt_old = 0.0;
double gps_vertical_speed, gps_alt_time = 0.0;

ExpRunAverFilter gps_vy_filter(0.1, 0.0);
ExpRunAverFilter gps_alt_filter(0.1, 0.0);
void reset_gps(){
...
float relative_altitude_gps(){
   return gps.altitude.meters() - gps_zero_alt;
}

void calibrate_gps_alt()
{
  double buf = 0.0, alt_old = 0.0;
  for(int i = 0; i < CALIBRATE_ZERO_BUF_SIZE; ++i){
    double alt = altitude_gps();
    while(alt != alt_old){
      buf += alt;
      alt_old = alt;
    }
  }
  gps_zero_alt = buf/double(CALIBRATE_ZERO_BUF_SIZE);
}

double get_gps_vertical_speed(){
  double cur_time = micros();
  double vy = (gps_flying_alt_old - gps_flying_alt)/((gps_alt_time - cur_time)/1000000.0);
  gps_flying_alt_old = gps_flying_alt;
  gps_alt_time = cur_time;
  return vy;
}

double gps_vy_filter() {
  static double filVal = 0.0;
  double k = 0.1; //коэффициент сглаживания (0.0 – 1.0), чем он меньше, тем плавнее фильтр
  double newVal = get_gps_vertical_speed();
  
  filVal += (newVal - filVal) * k;
  return filVal;
}

void update_flying_alt_gps(){
  gps_flying_alt = gps_alt_filter.get_filtered(relative_altitude_gps()); 
}
